...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 4 and sys.argv[3] == "bm25":
        ...
        # print('evaluating ...')
        # print("gold_ref")
        # print(len(golden_ref_amr))
        # print("answer:")
        # print(len(answers_amr))
        ...
        for ref_indx, ref in enumerate(golden_ref_amr):
            index_amr = -1
            prompt = ""
            # print(golden_ref_string[ref_indx])
            if "Answer V4_Q1_env" in golden_ref_string[ref_indx][0]:
                index_amr = 0
            elif "Answer V4_Q1_sou" in golden_ref_string[ref_indx][0]:
                index_amr = 1
            elif "Answer V4_Q1_soil" in golden_ref_string[ref_indx][0]:
                index_amr = 2

            # for index_amr in range(len(answers_amr)):
            if index_amr != -1:
                print("Given a student answer and a golden answer. Please evaluate the student answer against the golden answer, and list the key missing concepts from the student answer. Please write the final answer as this format:\nThe key missing concepts from the student answer are:\n - <concept1>\n - <concept2>\n - <concept3>\n - ...")
                prompt += "Given a student answer and a golden answer. Please evaluate the student answer against the golden answer, and list the key missing concepts from the student answer. Please write the final answer as this format:\nThe key missing concepts from the student answer are:\n - <concept1>\n - <concept2>\n - <concept3>\n - ..."
                print("**Student Answer (in text):**")
                prompt += "\n**Student Answer (in text):**"
                ...
                print("**Student Answer (AMR Graph):**")
                prompt += "\n"+ "**Student Answer (AMR Graph):**"
                print(penman.encode(penman.decode(golden_ref_amr_lines[ref_indx])))
                prompt += "\n"+ penman.encode(penman.decode(golden_ref_amr_lines[ref_indx]))
                print()
                print("**Golden Answer (in text):**")
                prompt += "\n"+ "**Golden Answer (in text):**"
                print(hy)
                prompt += "\n"+ hy
                print("**Golden Answer (AMR Graph):**")
                prompt += "\n"+ "**Golden Answer (AMR Graph):**"
                print(penman.encode(penman.decode(answers_amr_lines[index_amr])))
                prompt += "\n"+ penman.encode(penman.decode(answers_amr_lines[index_amr]))
                print("************** INPUT PROMPT ****************")
                print(prompt)
